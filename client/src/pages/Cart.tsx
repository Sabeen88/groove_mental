...
const Cart = () => {
  ...
  const cartData = cart.data?.data?.cart;
  if (!cartData) return <div>Your cart is empty.</div>;

  // Only show cart if paymentStatus is not "paid"
  if (cartData.paymentStatus === "paid") {
    return (
      <div className="text-center py-8 text-green-600 font-bold text-xl">
        Payment already completed for this cart!
      </div>
    );
  }
